...
public class ActualizarComunicacionJSFBean extends CommonJSFBean{
    ...
    private Collection<SelectItem> listaUsuarioDependencia = new ArrayList<>();
    ...
    public void init () throws Exception {
        ...
        try {
            if (((String)super.getElObjectFromBinding("#{bindings.idAsignacion.inputValue}")).equals("1")) {
                Long codDependencia = Long.parseLong(new BigDecimal((String) super.getElObjectFromBinding("#{bindings.codDependenciaDestino.inputValue}")).toString());
                
                System.out.println("Filtra usuarios por codigo dependencia: " + codDependencia);
                System.out.println("session bean " + this.comunicacionEJB);
                
                List<SgdUsuario> usuariosDependencia = this.comunicacionEJB.getSgdUsuarioFindByDependencia(codDependencia);
                
                listaUsuarioDependencia = new ArrayList<SelectItem>();
                
                for (SgdUsuario usuario : usuariosDependencia) {
                    listaUsuarioDependencia.add(new SelectItem(usuario.getIdUsuario(), usuario.getNombreUsuario()));
                }
                
                System.out.println("Usuarios retornados: " + listaUsuarioDependencia.size());   
            }                        
        } catch (Exception e) {
            log.error("error inicializando variables: " + e.getMessage(), e);
        }
        
        //Pobla lista se tipo de asignacion
        ...
        System.out.println("Obtiene nombre de la dependencia remitente con codigo: " + 
        ...
        System.out.println("carga plantillas");
        ...
        //port = ServiceLocator.getInstance().getExpMineroPort("http://10.0.100.254/sbresource?PROXY/ANMProject/Proxy%20Services/ExpedienteMineroProxy");
     ...
     * *************************************************/
        ...
        log.info(user + " -> esTituloMinero: "+getElObjectFromBinding("#{bindings.esTituloMinero.inputValue}"));
        if (getElObjectFromBinding("#{bindings.esTituloMinero.inputValue}") != null) {
    ...
    private void precargarMetadatos() {
        ...
        try {
            ...
            tramitesSet = this.wcContentEJB.getTramiteFindAll();
            for (Tramite f : tramitesSet) {
                tramites.add(new SelectItem(f.getIdtramite().toString(), f.getTramite()));
            }
            log.debug(user + " -> tramites size: " + tramites.size());
    ...
    private void cargarTiposDocumentales(String tramiteSelected) {
        ...
        try {
            tiposDocumentalesSet = wcContentEJB.getTipoDocumentalTramiteTiposDocumentalesTramites(new BigDecimal(tramiteSelected));
            log.debug(user + " -> listaTiposDocum size: " + tiposDocumentalesSet.size());
            tiposDocumentales.clear();
            for (TipoDocumentalTramite f : tiposDocumentalesSet) {
                tiposDocumentales.add(new SelectItem(f.getIdtipodtal().toString(), f.getNombretipodocumental()));
            }
            log.debug(user + " -> lstTiposDocum size: " + tiposDocumentales.size());
        } catch (Exception e) {
    ...
    private void precargarTaxonomia() {
        ...
        try {
            ...
            categoriasProcesoSet = this.wcContentEJB.getFoldersFindByParentGuid(PARENT_TAX_NAME);
            if (categoriasProcesoSet.size() > 0) {
                Folders parent = categoriasProcesoSet.iterator().next();
                categoriasProcesoSet = this.wcContentEJB.getFoldersFindByParentGuid(parent.getFfolderguid());
                log.debug(user + " -> categoriasProcesoSet size: " + categoriasProcesoSet.size());
                for (Folders f : categoriasProcesoSet) {
                    categoriasProceso.add(new SelectItem(f.getFfolderguid(), f.getFfoldername()));
                }
                log.debug(user + " -> categoriasProceso size: " + categoriasProceso.size());
            }
    ...
    private void cargarProcesos(String catProcesoSel) {
        ...
        try {
            procesosSet = this.wcContentEJB.getFoldersFindByParentGuid(catProcesoSel);

            log.debug(user + " -> procesosSet size: " + procesosSet.size());
            for (Folders f : procesosSet) {
                procesos.add(new SelectItem(f.getFfolderguid(), f.getFfoldername()));
            }
            log.debug(user + " -> procesos size: " + procesos.size());

            //Setear nombre de la categoria
            for (Folders item : categoriasProcesoSet) {
    ...
    private void cargarSeries(String idProceso) {
        ...
        try {
            ...
            seriesSet = wcContentEJB.getFoldersFindByParentGuid(idProceso);
            log.debug(user + " -> seriesSet size: " + seriesSet.size());
            for (Folders f : seriesSet) {
                series.add(new SelectItem(f.getFfolderguid(), f.getFfoldername()));
            }
            log.debug(user + " -> series size: " + series.size());

            //Setear nombre del proceso
            for (Folders item : procesosSet) {
    ...
    private void cargarSubseries(String idSerie) {
        ...
        try {
            ...
            subseriesSet = wcContentEJB.getFoldersFindByParentGuid(idSerie);
            log.debug(user + " -> subseriesSet size: " + subseriesSet.size());
            for (Folders f : subseriesSet) {
                subseries.add(new SelectItem(f.getFfolderguid(), f.getFfoldername()));
            }
            log.debug(user + " -> subseries size: " + subseries.size());
        } catch (Exception e) {
    ...
    private void cargarCuadernos(String idSubSerie) {
        ...
        try {
            ...
            cuadernosSet = wcContentEJB.getFoldersFindByParentGuid(idSubSerie);
            log.debug(user + " -> cuadernosSet size: " + cuadernosSet.size());
            for (Folders f : cuadernosSet) {
                cuadernos.add(new SelectItem(f.getFfolderguid(), f.getFfoldername()));
            }
            log.debug(user + " -> cuadernos size: " + cuadernos.size());
        } catch (Exception e) {
    ...
    public void cambiarAsignacion (ValueChangeEvent evt) {
        log.debug("Inicio cambiarAsignacion():"+evt.getOldValue());
        ...
        log.debug("Fin cambiarAsignacion():"+evt.getNewValue());
    }
    ...
    public void cambiarRequiereRespuesta (ValueChangeEvent evt) {
        ...
        try {
            ...
            log.error("error cambiando requiere respuesta: " + e.getMessage(), e);
        }
    ...
    public void cambiarDependenciaDestino ( ValueChangeEvent evt ){
        ...
        try{
                ...
                for (UnidadAdministrativa depDestino : dependenciasSet) {
                    if (newValue.equals(depDestino.getCodUnidadadministrativa())) {
                        ...
                        if (((String)super.getElObjectFromBinding("#{bindings.idAsignacion.inputValue}")).equals("1")) {
                            ...
                            this.listaUsuarioDependencia = new ArrayList<SelectItem>();
                            
                            for (SgdUsuario usuario : usuariosSet) {
                                this.listaUsuarioDependencia.add(new SelectItem(usuario.getIdUsuario(), usuario.getNombreUsuario()));
                            }
                            
                            log.debug("Usuarios retornados: " + this.listaUsuarioDependencia.size());   
                        }                        
                        break;
                    }                    
                }
            ...
            log.error("error cambiando dependencia: " + e.getMessage(), e);
        }        
            ...
            if (selectedValue != null) {
                ...
                for (SgdUsuario usuario : usuariosSet) {
                    if (selectedValue.equals(usuario.getIdUsuario())) {
                        ...
                        System.out.println("id usuario seleccionado: " + usuario.getIdUsuario());
                        System.out.println("nombre usuario seleccionado: " + usuario.getNombreUsuario());
                        
                        break;
                    }
                }
                
                log.debug("id asignado: " + super.getElObjectFromBinding("#{bindings.idUsuarioCom.inputValue}"));
    ...
    public void cambiarEsExpMinero (ValueChangeEvent evt) {
        ...
        try {
            ...
            log.error("error cambiando expediente minero: " + e.getMessage(), e);
        }
    }    
    ...
    public void cambiarCuaderno ( ValueChangeEvent evt ){
        log.debug("Inicio: cambiarCuaderno()");
    ...
    public void cambiarTramite ( ValueChangeEvent evt ){
        ...
        for ( Tramite t:this.tramitesSet ){
            if ( tramiteSelected.equals(t.getIdtramite().intValue()) ){
                super.setElObjectIntoBinding("#{bindings.idTramite.inputValue}", tramiteSelected);
        ...
        if (evt==null|| evt.getNewValue()==null){            
            ...
            for ( TipoDocumentalTramite t:this.tiposDocumentalesSet ){
                if ( tipoDocSele.equals(t.getIdtipodtal().toString()) ){
                    ...
                    super.setElObjectIntoBinding("#{bindings.idTpDocumento.inputValue}", tipoDocSele);
    ...
    public void validarExpMinero (ValueChangeEvent event) {
        ...
        try {   
            ...
            log.error("error valiando expediente minero: " + e.getMessage(), e);
        }
    }        
    
    public void actualizarComunicacion ( ActionEvent evt ){   
        log.info("INICIO actualizarComunicaci�n");
        ...
        try {
            ...
            */
            ...
            log.error("error actualizando comunicacion: " + e.getMessage(), e);
        }
        
        super.setOperation(evt);
        log.info("FIN actualizarComunicaci�n");
    }
